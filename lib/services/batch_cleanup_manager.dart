...
class BatchCleanupManager {
  ...
  // Cleanup thresholds
  static const Duration _offlineThreshold = Duration(minutes: 5); // Devices offline for 5+ minutes
  static const Duration _alertRetentionPeriod = Duration(hours: 24); // Keep alerts for 24 hours
  static const Duration _monitorOfflineThreshold = Duration(minutes: 10); // Monitors offline for 10+ minutes
  static const Duration _cleanupInterval = Duration(minutes: 15); // Run cleanup every 15 minutes
  ...
  void startBatchCleanup({
    ...
    if (kDebugMode) {
      print('BatchCleanupManager: Batch cleanup started with ${interval.inMinutes}분 간격');
    }
  }
  ...
  Future<void> _performBatchCleanup() async {
    if (_database == null) {
      if (kDebugMode) {
        print('BatchCleanupManager: Database not initialized, skipping cleanup');
      }
      return;
    }
    ...
    try {
      ...
      if (kDebugMode) {
        print('BatchCleanupManager: Cleanup completed in ${duration.inSeconds}s');
  ...
  Future<int> _cleanupOfflineMonitors() async {
    ...
    try {
      ...
      if (monitorsSnapshot?.exists == true) {
        final monitors = Map<String, dynamic>.from(monitorsSnapshot!.value as Map);
        final now = DateTime.now().millisecondsSinceEpoch;
        ...
        if (rootData != null) {
          ...
          for (final collection in ['devices', 'monitors', 'alerts']) {
            ...
            if (collectionData == null || 
              ...
              if (kDebugMode) {
                print('BatchCleanupManager: Removed empty collection: $collection');
              }
            }
          }
        }
